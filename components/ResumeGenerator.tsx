...
const ResumeGenerator: React.FC = () => {
  ...
  const shareViaWhatsApp = () => {
    // Garantimos que a URL seja absoluta usando location.origin
    // Adicionamos o parâmetro ?view=RESUME para o App.tsx interceptar
    const shareUrl = window.location.origin + '/?view=RESUME';
    const message = `Olá! Use este link para preencher seus dados e gerar seu currículo profissional com IA na ANIX COPIADORA: ${shareUrl}`;
    ...
    <div className="max-w-[1700px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 animate-fade-in pb-20">
      ...
      <div className="lg:col-span-7 space-y-6">
        <div className="flex items-center justify-between no-print sticky top-20 z-20 bg-slate-50/80 backdrop-blur-sm py-2 px-1">
          ...
          <div className="flex gap-2">
            <button onClick={shareViaWhatsApp} className="bg-emerald-500 text-white px-4 py-4 rounded-2xl font-black shadow-lg hover:bg-emerald-600 flex items-center gap-2 uppercase tracking-widest text-[9px] transition-all border border-emerald-400">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 .018 5.394 0 12.03a11.972 1.03 1.605 6.057L0 24l6.132-1.61a11.874 11.874 0 005.915 1.569h.005c6.632 0 12.028-5.398 12.03-12.03a11.85 11.85 0 00-3.48-8.502z"/></svg>
              Compartilhar Link
            </button>
            {resume && (
              <div className="flex gap-2">
                <button onClick={exportToWord} className="bg-white text-slate-700 px-4 py-4 rounded-2xl font-black shadow-sm hover:bg-slate-100 flex items-center gap-2 uppercase tracking-widest text-[9px] transition-all border border-slate-200">
                  Word (.doc)
                </button>
                <button onClick={handlePrint} className="bg-indigo-600 text-white px-6 py-4 rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 flex items-center gap-2 uppercase tracking-widest text-[9px] transition-all hover:-translate-y-1">
                  Imprimir
                </button>
              </div>
            )}
