...
const PhotoGenerator: React.FC = () => {
  ...
  const generatePreview = () => {
    ...
    img.onload = () => {
      ...
      if (ctx) {
        ...
        const frameAspectRatio = FRAME_WIDTH / FRAME_HEIGHT;
        let drawWidth, drawHeight, drawX, drawY;
        if (imgAspectRatio > frameAspectRatio) {
          drawHeight = FRAME_HEIGHT * zoom;
          drawWidth = drawHeight * imgAspectRatio;
        } else {
          drawWidth = FRAME_WIDTH * zoom;
          drawHeight = drawWidth / imgAspectRatio;
        }
        drawX = (FRAME_WIDTH - drawWidth) / 2 + position.x;
        drawY = (FRAME_HEIGHT - drawHeight) / 2 + position.y;
  ...
  }, [isCropping, rawImage, zoom, brightness, contrast, position]);
          ...
          <div className="lg:col-span-5 space-y-6">
                ...
                <div className="h-full flex flex-col bg-slate-900">
                  ...
                  <div className="p-6 bg-slate-950 border-t border-white/5 space-y-4">
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-black text-white uppercase tracking-widest w-16">Zoom</span>
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-black text-white uppercase tracking-widest w-16">Brilho</span>
                      <input type="range" min="0" max="200" step="1" value={brightness} onChange={(e) => setBrightness(Number(e.target.value))} className="flex-1 accent-indigo-500 h-2 rounded-lg" />
                    </div>
                    <div className="flex items-center gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <label className="bg-white border-2 border-slate-200 text-slate-800 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest cursor-pointer text-center hover:bg-slate-50 transition-all">
                Abrir Arquivo
                <input type="file" className="hidden" accept=".png, .jpg, .jpeg, .gif, .webp, .tiff, .svg, image/*" onChange={handleFileUpload} />
              </label>
            </div>
            {image && !isCropping && (
