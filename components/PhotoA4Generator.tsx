...
const PhotoA4Generator: React.FC = () => {
        ...
        <div className="flex items-center gap-4 flex-1 overflow-x-auto min-w-0 hide-scrollbar pb-1 pt-1">
          ...
          <div className="flex items-center gap-3 bg-slate-800/50 px-3 py-1 rounded-lg border border-slate-700 shrink-0">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              ><svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M20 12H4" /></svg></button>
        ...
        if (src) {
           ...
           img.onload = () => {
             ...
             updatePhotosState(prev => [...prev, {
                ...
                brightness: 100,
                contrast: 100,
                fitMode: 'contain' // Also default to contain so nothing gets cropped!
             }]);
           };
           img.src = src;
        }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        if (e.key === 'a' || e.key === 'A') {
          ...
          e.preventDefault();
          if (e.shiftKey) {
             // Redo if we had it, but for now just undo on Ctrl+Z
          } else {
             handleUndo();
          }
        }
    ...
    if (e.button === 2) {
      ...
      if (photo) {
        if (e.altKey && !selectedIds.includes(photoId)) {
           ...
           // se clicar numa avulsa com alt, já duplica e arrasta
           e.preventDefault();
           e.stopPropagation();
           const newId = Math.random().toString(36).substr(2, 9);
           const duplicatePhoto = { ...photo, id: newId };
           updatePhotosState(prev => [...prev, duplicatePhoto]);
           setSelectedIds([newId]);
           setDraggingId(newId);
           const clientX = 'touches' in e ? (e as React.TouchEvent).touches[0].clientX : (e as React.MouseEvent).clientX;
           const clientY = 'touches' in e ? (e as React.TouchEvent).touches[0].clientY : (e as React.MouseEvent).clientY;
           setDragStart({ x: clientX, y: clientY, initialPosX: duplicatePhoto.posX, initialPosY: duplicatePhoto.posY });
        } else {
  ...
  const fillPageWithSelected = () => {
    ...
    selected.forEach(photo => {
       ...
       const cols = Math.max(1, Math.floor((availableW + gapMm) / (pW + gapMm)));
       const rows = Math.max(1, Math.floor((availableH + gapMm) / (pH + gapMm)));
  ...
  const clearAll = () => {
     if(window.confirm("Deseja realmente remover todas as fotos?")) {
        updatePhotosState([]);
        setSelectedIds([]);
     }
  };
  ...
  const handleResizeStart = (e: React.MouseEvent | React.TouchEvent, photo: PhotoItem, handle: 'br' | 'tl' | 'tr' | 'bl') => {
    ...
    let centerX = 0;
    let centerY = 0;
    let initialDistanceX = 1;
    let initialDistanceY = 1;
    
    if (container) {
      const rect = container.getBoundingClientRect();
      centerX = rect.left + rect.width / 2;
      centerY = rect.top + rect.height / 2;
      initialDistanceX = Math.abs(clientX - centerX) || 1;
      initialDistanceY = Math.abs(clientY - centerY) || 1;
    }
    
    setResizeStart({ 
      x: clientX, 
      y: clientY, 
      initialWidth: photo.widthMm, 
      initialHeight: photo.heightMm, 
      handle,
      centerX,
      centerY,
      initialDistanceX,
  ...
  const handleGlobalResizeMove = (e: MouseEvent | TouchEvent) => {
    ...
    const newZoomX = resizeStart.initialZoomX * (currentDistanceX / resizeStart.initialDistanceX);
    const newZoomY = resizeStart.initialZoomY * (currentDistanceY / resizeStart.initialDistanceY);
  ...
  const generatePageCanvas = async (pageIndex: number) => {
    ...
    for (const photo of pagePhotos) {
      ...
      if (!isFullBleed && currentX + pW > (w - marginMm * dpiFactor + 1)) { currentX = marginMm * dpiFactor; currentY += currentRowHeight + (gapMm * dpiFactor); currentRowHeight = 0; }
      
      let drawX = isFullBleed ? 0 : currentX;
      let drawY = isFullBleed ? 0 : currentY;

      let img;
      if (photo.isTextNode || photo.isWordArt) {
          // Fallback text drawing if needed, but since WordArt is SVG inside photo.src, it works the same!
          // We will wait for image load just in case.
          img = new Image(); img.src = photo.src;
          if(photo.src) await new Promise<void>(resolve => { img.onload = () => resolve(); img.onerror = () => resolve(); });
      } else {
          img = new Image(); img.src = photo.src;
      ...
      if (!photo.isTextNode) {
         ...
         if (img && img.width) {
             const imgRatio = img.width / img.height;
             const isRotated = (photo.rotation || 0) % 180 !== 0;
             const targetW = isRotated ? clipH : clipW;
             const targetH = isRotated ? clipW : clipH;
             const targetRatio = targetW / targetH;
             const fitMode = photo.fitMode || 'cover';
             
             let drawW, drawH;
             if (fitMode === 'cover') {
                if (imgRatio > targetRatio) { drawH = targetH; drawW = img.width * (targetH / img.height); } 
                else { drawW = targetW; drawH = img.height * (targetW / img.width); }
             } else if (fitMode === 'fill') {
                drawW = targetW; drawH = targetH;
             } else {
                if (imgRatio > targetRatio) { drawW = targetW; drawH = img.height * (targetW / img.width); } 
                else { drawH = targetH; drawW = img.width * (targetH / img.height); }
             }
             
             const centerX = clipX + (clipW / 2); const centerY = clipY + (clipH / 2);
             ctx.translate(centerX + ((photo.posX || 0) * (dpiFactor / mmToPxPreview)), centerY + ((photo.posY || 0) * (dpiFactor / mmToPxPreview)));
             ctx.rotate(((photo.rotation || 0) * Math.PI) / 180);
             ctx.scale((photo.zoomX ?? photo.zoom ?? 1) * (photo.flipX ? -1 : 1), (photo.zoomY ?? photo.zoom ?? 1) * (photo.flipY ? -1 : 1));
        ...
        // Simple plain text drawing (fallback, normally they use WordArt)
        ctx.fillStyle = photo.textColor || '#000000';
        ctx.font = `${(photo.fontSize || 24) * (dpiFactor / mmToPxPreview)}px ${photo.fontFamily || 'Inter'}`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.translate(pW/2 + ((photo.posX || 0) * (dpiFactor / mmToPxPreview)), pH/2 + ((photo.posY || 0) * (dpiFactor / mmToPxPreview)));
        ctx.rotate(((photo.rotation || 0) * Math.PI) / 180);
        ctx.scale(photo.zoom || 1, photo.zoom || 1);
        ctx.fillText(photo.textContent || '', 0, 0);
      }
  ...
  const exportToPdf = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        if (i > 0) pdf.addPage();
        const canvas = await generatePageCanvas(i);
        if (canvas) {
           const imgData = canvas.toDataURL('image/jpeg', 0.95);
           const w = orientation === 'portrait' ? pageSize.w : pageSize.h;
           const h = orientation === 'portrait' ? pageSize.h : pageSize.w;
           pdf.addImage(imgData, 'JPEG', 0, 0, w, h);
        }
      }
      pdf.save('Anix_Fotos.pdf');
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const renderPhotoContent = (photo: PhotoItem, isPrint: boolean) => {
    ...
    // Se a foto é grande o suficiente para preencher a página, puxamos ela para cobrir a margem
    const isFullBleed = photo.widthMm >= pageW - 1 && photo.heightMm >= pageH - 1;
    ...
    return (
      <div key={photo.id} id={"photo-" + photo.id}
        ...
        {photo.isTextNode ? (
          ...
          <div dangerouslySetInnerHTML={{ __html: photo.src }} className="absolute inset-0 w-full h-full object-contain pointer-events-none z-10" style={{ transform: `scale(${photo.zoom || 1}) translate(${(photo.posX || 0)}px, ${(photo.posY || 0)}px) rotate(${photo.rotation || 0}deg)`, transformOrigin: 'center' }} />
           ...
           <div 
             ...
             style={{ 
                 left: isPolaroid ? `${POLAROID_SIDE_MARGIN_RATIO * 100}%` : '0',
                 right: isPolaroid ? `${POLAROID_SIDE_MARGIN_RATIO * 100}%` : '0',
                 top: isPolaroid ? `${(width * POLAROID_TOP_MARGIN_RATIO) / height * 100}%` : '0',
                 bottom: isPolaroid ? `${POLAROID_BOTTOM_MARGIN_RATIO * 100}%` : '0',
                 borderRadius: isRound && !photo.isTextNode ? '50%' : '0'
             }}
           >
             {(() => {
                const isRotated = (photo.rotation || 0) % 180 !== 0;
                const clipWValue = isPolaroid ? width * (1 - POLAROID_SIDE_MARGIN_RATIO * 2) : width;
                const clipHValue = isPolaroid ? height - (width * POLAROID_TOP_MARGIN_RATIO) - (height * POLAROID_BOTTOM_MARGIN_RATIO) : height;

                return (
                 <div 
                   className="absolute pointer-events-none"
                   style={{ 
                     width: isRotated ? `${clipHValue}${unit}` : '100%',
                     height: isRotated ? `${clipWValue}${unit}` : '100%',
                     left: '50%',
                     top: '50%',
                     transform: `translate(-50%, -50%) translate(${(photo.posX || 0)}px, ${(photo.posY || 0)}px) rotate(${photo.rotation || 0}deg)`,
                   }}
                 >
                   <img 
                     src={photo.src} 
                     ...
                     style={{ 
                       ...
                       transform: `scaleX(${(photo.zoomX ?? photo.zoom ?? 1) * (photo.flipX ? -1 : 1)}) scaleY(${(photo.zoomY ?? photo.zoom ?? 1) * (photo.flipY ? -1 : 1)})`,
        ...
        {!isPrint && selectedIds.includes(photo.id) && !activeCropId && (
          <>
            <div style={{ position: 'absolute', right: 0, bottom: 0, width: '12px', height: '12px', backgroundColor: '#3b82f6', cursor: 'se-resize', zIndex: 9999, borderRadius: '2px' }} onMouseDown={(e) => handleResizeStart(e, photo, 'br')} />
            <div style={{ position: 'absolute', left: 0, top: 0, width: '12px', height: '12px', backgroundColor: '#3b82f6', cursor: 'nw-resize', zIndex: 9999, borderRadius: '2px' }} onMouseDown={(e) => handleResizeStart(e, photo, 'tl')} />
            <div style={{ position: 'absolute', right: 0, top: 0, width: '12px', height: '12px', backgroundColor: '#3b82f6', cursor: 'ne-resize', zIndex: 9999, borderRadius: '2px' }} onMouseDown={(e) => handleResizeStart(e, photo, 'tr')} />
  ...
  return (
    <div className="flex w-full h-[calc(100vh-64px)] overflow-hidden bg-slate-200 font-sans text-slate-900 border-t border-slate-300">
      ...
      >
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar p-3 pt-0 flex flex-col gap-4 ${!isSidebarExpanded ? 'hidden' : ''}`}>
           ...
           <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={() => window.print()} className="flex flex-col items-center justify-center p-3 rounded-xl bg-violet-500 hover:bg-violet-600 text-white shadow-sm transition-all shadow-violet-500/20 w-full group col-span-2">
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mb-1.5 opacity-90 group-hover:opacity-100 group-active:scale-95 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                 <span className="text-[9px] font-black uppercase tracking-wider">Imprimir Direto</span>
              </button>
           </div>
                     ...
                     {!isPolaroid && selectedIds.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                           {[ {l:"3x4", w:30, h:40}, {l:"A6", w:105, h:148}, {l:"A5", w:148, h:210}, {l:"A4", w:210, h:297} ].map(size => (
                             <button 
                               key={size.l} 
                               onClick={() => updatePhotosState((photos: any) => photos.map((p: any) => {
                                 if (!selectedIds.includes(p.id)) return p;
                                 const isLandscape = p.widthMm > p.heightMm;
                                 return {
                                   ...p,
                                   widthMm: isLandscape ? size.h : size.w,
                                   heightMm: isLandscape ? size.w : size.h
                                 };
                               }))}
               ...
               <div className="toolbar-group">
                 ...
                 <div className="flex flex-col gap-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                       ...
                       <button onClick={fillPageWithSelected} disabled={selectedIds.length === 0} className="bg-indigo-100 text-indigo-700 border border-indigo-200 hover:bg-indigo-200 py-1.5 rounded-lg text-[8px] sm:text-[9px] font-bold uppercase tracking-wider flex items-center justify-center disabled:opacity-40 transition-colors" title="Preencher a página atual com cópias da foto selecionada">
                         📄<span className="ml-1 shrink-0">Preencher</span>
                       </button>
                       <button onClick={removeSelected} disabled={selectedIds.length === 0} className="bg-rose-100 text-rose-700 border border-rose-200 hover:bg-rose-200 py-1.5 rounded-lg text-[8px] sm:text-[9px] font-bold uppercase tracking-wider flex items-center justify-center disabled:opacity-40 transition-colors">
                         🗑️<span className="ml-1 shrink-0">Excluir</span>
                       </button>
      ...
      >
        {isDraggingOver && (
            <div className="absolute inset-0 z-50 bg-indigo-500/10 border-4 border-indigo-500 border-dashed rounded-xl flex items-center justify-center pointer-events-none m-4">
                <div className="bg-white px-6 py-4 rounded-full shadow-2xl flex items-center gap-3">
                    <span className="text-2xl">📥</span>
                    <span className="text-sm font-black text-indigo-700 uppercase">Solte suas imagens aqui</span>
                </div>
            </div>
        )}
          ...
          <button 
            onClick={() => { 
                updatePhotosState((photos: any) => photos.map((p: any) => selectedIds.includes(p.id) ? { 
                  ...p, 
                  rotation: (p.rotation || 0) + 90, 
                  widthMm: isPolaroid ? p.widthMm : p.heightMm, 
                  heightMm: isPolaroid ? p.heightMm : p.widthMm 
                } : p));
                setContextMenu(prev => ({ ...prev, visible: false })); 
            }}  
            disabled={selectedIds.length === 0}
