  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in pb-12">
      ...
      <div className="print-only">
        ...
        >
          {template === 'PIX_STANDARD' && !bgImage && (
            <div style={{ backgroundColor: bgColor, position: 'absolute', inset: 0, zIndex: 0 }}>
              <div style={{ position: 'absolute', bottom: '-150px', left: '-50%', width: '200%', height: '450px', backgroundColor: '#35D31D', borderRadius: '100%', opacity: 0.9 }}></div>
            </div>
          )}
          ...
          {bgImage && <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.15)', zIndex: 0 }}></div>}
          
          <div style={{ marginTop: '4mm', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '100%', zIndex: 10 }}>
            {template === 'PIX_STANDARD' && !bgImage ? (
              <div style={{ textAlign: 'center' }}>
                <span style={{ fontSize: '24px', fontWeight: 900, letterSpacing: '-0.5px', color: 'white', textTransform: 'uppercase' }}>PAGUE COM</span>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginTop: '8px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px', transform: 'rotate(45deg)' }}>
                    <div style={{ width: '16px', height: '16px', backgroundColor: 'white' }}></div>
                    <div style={{ width: '16px', height: '16px', backgroundColor: 'white' }}></div>
                    <div style={{ width: '16px', height: '16px', backgroundColor: 'white' }}></div>
                    <div style={{ width: '16px', height: '16px', backgroundColor: 'white' }}></div>
                  </div>
                  <span style={{ fontSize: '60px', fontWeight: 900, color: 'white', fontStyle: 'italic', letterSpacing: '-2px' }}>PIX</span>
                </div>
              </div>
            ) : (
              <span style={{ fontSize: '36px', fontWeight: 900, fontStyle: 'italic', letterSpacing: '-1px', textTransform: 'uppercase', color: bgColor.toLowerCase() === '#fcfc30' && !bgImage ? '#003399' : 'white' }}>
                {currentTpl.name}
              </span>
            )}
          </div>
          
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', zIndex: 10 }}>
            {template !== 'PIX_STANDARD' && (
              <h1 style={{ fontSize: '20px', fontWeight: 900, color: bgColor.toLowerCase() === '#fcfc30' && !bgImage ? '#003399' : 'white', textTransform: 'uppercase', marginBottom: '32px', letterSpacing: '0.2em', textAlign: 'center' }}>PAGUE VIA PIX</h1>
            )}
            
            <div style={{ backgroundColor: 'white', padding: '24px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', width: '64mm', height: '64mm', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: '2px solid rgba(255,255,255,0.5)', borderRadius: template === 'PIX_STANDARD' ? '0' : '12px', marginTop: template === 'PIX_STANDARD' ? '16px' : '0' }}>
              {qrCode && <img src={qrCode} style={{ width: '100%', height: '100%', objectFit: 'contain' }} alt="QR" />}
            </div>
            
            {pixKey.trim() && (
              <div style={{ width: '100%', marginTop: '32px' }}>
                <div style={{ width: '100%', padding: '20px 24px', borderRadius: '12px', textAlign: 'center', backgroundColor: 'rgba(0,0,0,0.2)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <p style={{ fontSize: '9px', fontWeight: 900, textTransform: 'uppercase', marginBottom: '4px', opacity: 0.8, color: 'white', letterSpacing: '0.2em' }}>CHAVE PIX</p>
                  <p style={{ fontSize: '13px', fontWeight: 900, lineHeight: 1.4, color: 'white', letterSpacing: '-0.5px', whiteSpace: 'pre-wrap' }}>{pixKey}</p>
                </div>
              </div>
            )}
          </div>
